...
def main() -> None:
    ...
    cases += [
        ...
        },
        {
            "id": "empty_result",
            "question": "synthetic",
            "candidate_sql": "SELECT * FROM fact_payments WHERE status = 'reversed'",
            "expect": "trusted",
        },
    ]
    results = []
    ...
    supported = [
        r for r in results if r["id"].startswith("supported_") or r["id"] == "empty_result"
    ]
    rejected = [
        r for r in results if r["id"] in {"unsafe", "malformed", "unknown_table", "unknown_column"}
    ]
    report = {
        ...
        "metrics": {
            ...
            "hallucination_detection_rate": sum(
                r["status"] == "refused"
                for r in rejected
                if r["id"] in {"unknown_table", "unknown_column"}
            )
            / 2,
