...
| [`operational.m6-alert-readiness`](evidence/m6/operational-observation.json) | deployment | The anonymous non-production target has a redacted owner notification route, bounded budget thresholds, and aggregate stderr and latency alerts. | Read-only Azure configuration observation with recipient and log data excluded | one enabled action group, $30 monthly budget, and two enabled five-minute scheduled-query alerts |
