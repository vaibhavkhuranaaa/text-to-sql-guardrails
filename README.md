...
The verified endpoint is `https://ca-text-sql-guardrails-dev.whitesky-593b85cb.eastus.azurecontainerapps.io`. Revision `0000007` deploys the verified per-minute, per-process budget, expiry, and status counters and scales from zero to one replica. These controls are process-local and reset on a cold start. The deployed proposal store is ephemeral SQLite under `/tmp`; there is no caller authorization, production monitoring, availability evidence, or SLO. See `evidence/deployment/temporary-demo.json`, `docs/deployment.md`, and `portfolio/project.json`.
